...
const TaskHeader = ({
	...
	return (
		<div className="group pt-2 pb-0 px-3">
			...
			<div className={cn("relative grid items-start gap-2", hasTodos && "min-[760px]:grid-cols-2")}>
				<div data-testid="task-summary-slot" className="relative min-h-[64px] min-w-0 self-start">
					<div
						data-testid="task-summary-panel"
						className={cn(
							"self-start px-3 pt-2.5 pb-2 flex flex-col gap-1.5 relative z-1 cursor-pointer min-w-0",
							isTaskExpanded && "absolute inset-x-0 top-0 z-20 max-h-[min(70dvh,36rem)] overflow-y-auto",
						...
						onClick={(e) => {
							...
							setIsTaskExpanded((isExpanded) => !isExpanded)
						}}>
							...
							<div className="flex items-center select-none grow min-w-0">
								...
								<div className="flex items-center shrink-0 ml-2" onClick={(e) => e.stopPropagation()}>
									<StandardTooltip
										...
										<button
											onClick={() => setIsTaskExpanded((isExpanded) => !isExpanded)}
				...
				{hasTodos && (
					<div data-testid="todo-list-slot" className="relative self-start min-h-[48px] min-w-0">
						<TodoListDisplay
