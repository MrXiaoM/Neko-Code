...
export const ChatRowContent = ({
	...
	if (tool) {
			...
			switch (tool.tool as string) {
				...
				case "newTask":
					...
					const isFollowedBySubtaskResult =
						nextMessage?.type === "say" && nextMessage?.say === "subtask_result"
		...
		// Unrecognized tools fall through to `null` above; in that case we still
		// render just the gray tool-name label. Recognized tools get their rich
		// UI plus the label appended underneath.
		if (!toolContent) {
			return toolNameBadge
		}
