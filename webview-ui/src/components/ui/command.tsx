...
type CommandListProps = React.ComponentPropsWithoutRef<typeof CommandPrimitive.List> & {
	maxHeight?: string
	/** Uses the browser's native wheel handling instead of the WebView workaround. */
	nativeWheel?: boolean
}

const CommandList = React.forwardRef<React.ElementRef<typeof CommandPrimitive.List>, CommandListProps>(
	({ className, onWheel, maxHeight, nativeWheel = false, ...props }, ref) => {
		...
		return (
			<CommandPrimitive.List
				...
				onWheel={nativeWheel ? onWheel : handleWheel}
				{...props}
			/>
		)
	},
