	...
	implements vscode.WebviewViewProvider, TelemetryPropertiesProvider, TaskProviderLike
{
	...
	constructor(
		...
		// Task history is persisted to per-task files. Storing full history in VS Code
		// globalState causes costly state serialization during window reloads.
	...
	private async initializeTaskHistoryStore(): Promise<void> {
		try {
			...
			// The file-backed store is now authoritative. Remove the legacy mirror only
			// after it has initialized and any one-time migration has completed.
	...
	*/
	...
	private async getHMRHtmlContent(webview: vscode.Webview): Promise<string> {
		...
		const csp = [
			...
			`img-src ${webview.cspSource} https://storage.googleapis.com https://img.clerk.com https://avatars.githubusercontent.com https://lh3.googleusercontent.com data: http://${localServerUrl}`,
	...
	> {
		...
		return {
			...
			systemNotificationOnApproval: stateValues.systemNotificationOnApproval ?? true,
			systemNotificationOnOther: stateValues.systemNotificationOnOther ?? true,
			nekoNotifierDataDirectory: stateValues.nekoNotifierDataDirectory,
